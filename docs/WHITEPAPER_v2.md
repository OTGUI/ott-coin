...
| **소수점** | 6자리 |
| **발행 방식** | Phase 1: Pump.fun Fair Launch → Phase 2: 자체 LP (Raydium CLMM) |
| **핵심 유틸리티** | 보유자 자동 응원 NFT 에어드롭 |
| **NFT 기술** | Compressed NFT (cNFT) — Metaplex Bubblegum |
| **NFT 등급** | 5단계 (Common → Rare → Epic → Legendary → Mythic) |
| **NFT 한정** | Legendary 40개, Mythic 3개 (Phase 1 Pump.fun: Mythic 1개 한정) |
...
### 3.2 이중구조 (Dual Pool Architecture)

$OTT는 **2단계 유동성 구조**를 채택합니다. 이는 커뮤니티 빌딩과 지속가능한 생태계 운영을 동시에 달성하기 위한 설계입니다.

```
┌─────────────────────────────────────────────────┐
│              $OTT 이중구조 개요                    │
│                                                   │
│  Phase 1: Pump.fun (커뮤니티 빌딩 풀)              │
│  ├── 목적: 초기 커뮤니티 형성 & 인지도 확보          │
│  ├── 100% Fair Launch (프리세일/프리마이닝 없음)     │
│  └── 본딩커브 기반 자동 가격 결정                    │
│                                                   │
│           ↓ 원웨이 마이그레이션 (v1 → v2)           │
│                                                   │
│  Phase 2: Raydium CLMM (메인 유동성 풀)            │
│  ├── 목적: 장기 생태계 운영 & 지속가능한 수익 구조    │
│  ├── 팀 물량 확보 → 개발/운영 자금                  │
│  ├── LP 수수료 → 프로젝트 운영비                    │
│  └── Jupiter/Raydium/Birdeye 등 DEX 연동           │
│                                                   │
│  🔄 스왑: Pump.fun OTT → Raydium OTT (1:1 + 30% 보너스) │
│  ❌ 역방향 불가: Raydium OTT ↛ Pump.fun OTT       │
└─────────────────────────────────────────────────┘
```

### 3.3 Phase 1: Pump.fun Fair Launch (커뮤니티 빌딩)

$OTT는 **Pump.fun**을 통해 최초 공정 발행(Fair Launch)되었습니다. 프리세일, 프리마이닝, VC 투자가 없습니다.
...
- 초기 커뮤니티 참여자 확보
- $OTT 브랜드 인지도 구축
- Fair Launch를 통한 공정한 초기 분배
- Phase 2 마이그레이션을 통한 장기 홀더 보상

> **📢 Phase 1 참여자 혜택:** Pump.fun에서 구매한 $OTT를 Phase 2 Raydium 풀의 $OTT로 스왑하면 **1:1 교환 + 30% 추가 에어드롭**을 받습니다. 예: 100,000 OTT v1 → 130,000 OTT v2. 초기 참여자일수록 낮은 가격에 진입하여 더 많은 토큰을 확보할 수 있습니다.

### 3.4 Phase 2: Raydium CLMM (메인 유동성 풀)

Phase 1에서 충분한 커뮤니티가 형성된 후, $OTT의 **메인 유동성 풀**이 Raydium CLMM에 생성됩니다.

**분배 구조 (Phase 2 토큰):**

| 항목 | 비율 | 수량 | 용도 | 락업 |
|------|------|------|------|------|
| **마이그레이션 풀** | 50% | 1,215,000,000,000 | Phase 1 홀더 1:1 스왑 + 30% 보너스 | 스왑 요청 시 해제 |
| **유동성 풀 (LP)** | 20% | 486,000,000,000 | Raydium CLMM 초기 유동성 | LP 토큰 팀 보유 |
| **팀/운영** | 15% | 364,500,000,000 | 개발, 운영, 인프라 | **6개월 단계적 해제** |
| **에어드롭/마케팅** | 10% | 243,000,000,000 | NFT 이벤트, KOL, 캠페인 | 필요 시 해제 |
| **커뮤니티 리저브** | 5% | 121,500,000,000 | DAO 거버넌스, 미래 용도 | DAO 의결 시까지 잠금 |

**Phase 2의 핵심 차이점:**

| 항목 | Phase 1 (Pump.fun) | Phase 2 (Raydium) |
|------|--------------------|--------------------|
| LP 소유권 | Pump.fun (소각) | **팀 보유** |
| LP 수수료 수익 | 없음 | **팀 수익** → 운영비 |
| 팀 물량 | 없음 | **15%** (락업) |
| DEX 연동 | Pump.fun만 | Jupiter, Raydium, Birdeye 등 |
| 거래 | 본딩커브 | 자유 시장 |

> **🔑 LP 토큰 팀 보유 = 프로젝트 지속가능성.** LP 수수료가 운영비로 사용되며, 이는 에어드롭 시스템, 커뮤니티 이벤트, 개발 자금으로 투명하게 집행됩니다.

### 3.5 원웨이 마이그레이션 (v1 → v2 스왑)

Phase 1 Pump.fun 홀더를 Phase 2로 안전하게 전환하기 위한 **단방향 스왑 시스템**입니다.

```
┌──────────────┐         ┌──────────────┐
│  Pump.fun    │  1:1    │  Raydium     │  + 30% 보너스
│  (v1 OTT)   │  +30%   │  (v2 OTT)   │
│  $OTT (v1)   │ ──────→ │  $OTT (v2)   │
│              │         │              │
│  ❌ 역방향   │ ←─ ✕ ── │              │
...
**스왑 규칙:**
- ✅ Pump.fun OTT → Raydium OTT: **1:1 교환 + 30% 추가 에어드롭** (네트워크 수수료만 부담)
  - 예시: 100,000 v1 OTT → 100,000 v2 OTT + **30,000 v2 OTT 보너스** = 총 130,000 v2 OTT
- ❌ Raydium OTT → Pump.fun OTT: **불가**
- 스왑된 v1 토큰은 **영구 소각 (Burn)**

**스왑 방법:**
1. **텔레그램 봇** — `/swap` 명령어로 간편 스왑
2. **웹사이트** — ottcoin.life/swap 페이지에서 Phantom 월렛 연결 후 스왑
3. **스마트 컨트랙트** — 온체인 자동 스왑 (Phase 2 고도화 시)

**왜 단방향인가?**
- Phase 1은 커뮤니티 빌딩 목적으로 완료됨
- 역방향 스왑을 허용하면 차익거래(Arbitrage)로 두 풀의 균형이 깨짐
- 단방향 스왑으로 Phase 2에 유동성 집중 → 더 건강한 생태계
- **30% 보너스**는 초기 참여자에 대한 보상이자 Phase 2 마이그레이션 인센티브

> **⏰ 마이그레이션 기간:** Phase 2 오픈 후 **6개월간** 스왑 가능. 기간 만료 후 마이그레이션 풀의 미사용 토큰은 커뮤니티 리저브로 전환됩니다.
...
> ⚠️ **중요:** NFT 에어드롭은 **Phase 2(Raydium CLMM)** 시작 후 가동됩니다. Phase 1(Pump.fun)은 커뮤니티 빌딩 단계로, NFT는 발행되지 않습니다. Phase 1에서 $OTT를 보유한 홀더는 Phase 2 마이그레이션 후 NFT 에어드롭 대상에 포함되며, Gold Mythic 등 Phase 1 참여자 전용 보상 자격을 얻습니다.
...
| # | 이름 | 스타일 | 소재 | 풀 |
|---|------|--------|------|-----|
| 1 | 🥇 **Gold** | 엠보싱 금화 스타일 | 불굴의 의지 | **Phase 1 참여자 전용** (Phase 2에서 발행) |
| 2 | 💎 **Diamond** | 크리스탈/다이아몬드 텍스처 | 빛나는 재기 | Phase 2 (Raydium) |
| 3 | 🪙 **Platinum** | 양팔 만세 포즈, 플래티넘 색상 | 궁극의 승리 | Phase 2 (Raydium) |

- 3개 모두 **완전히 다른 아트 디렉션, 메시지, 스타일**
- 최고 보유자 또는 특별 이벤트를 통해서만 획득 가능
- 소유자에게 **프로젝트 거버넌스 투표권** 부여

> **🥇 Gold Mythic — Phase 1 참여자 전용:** Pump.fun(Phase 1)에서 $OTT를 보유하고 Phase 2로 마이그레이션한 홀더에게만 에어드롭 자격이 주어지는 유일한 Mythic입니다. **단 1개만** 존재하며, 극한의 확률로 당첨됩니다. Phase 1 초기 참여자만의 궁극의 보상입니다.
>
> ⚠️ **참고:** Phase 1(Pump.fun)에서는 NFT가 발행되지 않습니다. 모든 NFT 에어드롭은 Phase 2(Raydium) 시작 후 가동됩니다.
...
### Phase 1: 탄생 — Genesis (완료 ✅)

- [x] $OTT 토큰 Pump.fun Fair Launch 발행
- [x] 로고 및 브랜드 아이덴티티 확립
- [x] 랜딩 페이지 (ottcoin.life) 배포
- [x] 텔레그램 채널 (@OTTCOINLIFE) 개설
- [x] X(트위터) 계정 (@ottcoinlife) + 초기 콘텐츠
- [x] Discord 커뮤니티 서버 개설
- [x] DexScreener 자동 등록
- [x] Merkle Tree 생성 (16,384 NFT 용량)
- ℹ️ Phase 1에서는 NFT 미발행 — 커뮤니티 빌딩 집중

### Phase 2: 전환 — Migration (진행중 🔄)

- [ ] Raydium CLMM 메인 유동성 풀 생성
- [ ] $OTT v2 토큰 발행 (팀 물량 + LP 확보)
- [ ] 원웨이 스왑 시스템 구축 (v1 → v2)
- [ ] 백서/웹사이트 이중구조 반영
- [ ] **cNFT 에어드롭 시스템 가동** (Phase 2부터 시작)
- [ ] 텔레그램 봇 고도화 (/swap, /balance 등)
